...
func _update_focus_relations() -> void:
	...
	for hook_index in row_count:
		...
		var previous_hook: Control = (
			current_hook if hook_index == 0 else _hooks_list.get_child(hook_index - 1)
		)
		var next_hook: Control = (
			current_hook if hook_index >= row_count - 1 else _hooks_list.get_child(hook_index + 1)
		)
