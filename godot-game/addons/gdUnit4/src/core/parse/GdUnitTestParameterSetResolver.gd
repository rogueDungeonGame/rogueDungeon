...
func validate(parameter_sets: Array, parameter_set_index: int) -> GdUnitResult:
	...
	if input_values == null:
		return GdUnitResult.error(
			"The parameter set '%s' must be an Array!" % parameter_sets[parameter_set_index]
		)
	...
	if current_arg_count != expected_arg_count:
		...
		return (
			...
			"""
					...
					% [parameter_set_index, ",".join(arg_names), input_values]
				)
			)
		)
...
static func validate_parameter_types(
	input_arguments: Array[GdFunctionArgument], input_values: Array
) -> GdUnitResult:
		...
		if input_param_type != input_value_type:
			return (
				...
				"""
						...
						% [input_value, type_string(input_value_type), str(input_param)]
					)
				)
			)
...
func load_parameter_sets(test_suite: Node) -> GdUnitResult:
	...
	if result != OK:
		return GdUnitResult.error(
			"Extracting test parameters failed! Script loading error: %s" % error_string(result)
		)
