...
static func create_property_if_need(
	property_name: String, default_value: Variant, help_text := "", value_set := PackedStringArray()
) -> void:
...
static func set_property_info(
	property_name: String, value: Variant, value_set: PackedStringArray
) -> void:
	var info := {
		"name": property_name,
		"type": typeof(value),
...
static func validate_is_inferred_declaration_enabled() -> GdUnitResult:
	if (
		ProjectSettings.get_setting(GDSCRIPT_WARNINGS_INFERRED_DECLARATION)
		== GdScriptWarningMode.IGNORE
	):
		return GdUnitResult.success()

	if Engine.get_version_info().hex >= 0x40600:
		var directory_rules: Dictionary = ProjectSettings.get_setting(
			GDSCRIPT_WARNINGS_DIRECTORY_RULES
		)
		...
		var is_excluded: bool = (
			not best_match.is_empty()
			and directory_rules[best_match] == GdScriptWarningDirectoryMode.EXCLUDE
		)
		if not is_excluded:
			return (
				GdUnitResult
