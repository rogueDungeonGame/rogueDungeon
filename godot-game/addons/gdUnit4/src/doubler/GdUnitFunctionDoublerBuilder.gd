...
func build_func_signature() -> String:
	var return_type := (
		":" if fd._return_type == TYPE_VARIANT else " -> %s:" % fd.return_type_as_string()
	)
			...
			var signature := "{argument_name}{arg_typed}={arg_value}".format(
				{
					...
					"arg_typed":
					(
						":" + GdObjects.type_as_string(arg.type())
...
func build_arguments() -> String:
	return "\tvar __args := [{args}]{varargs}".format(
		{"args": ", ".join(func_args), "varargs": " + varargs_" if fd.is_vararg() else ""}
	)
...
func build_super_calls() -> String:
	...
	for index in range(0, 11):
		match_block += (
			"{index}: super({args})\n"
			. format({"index": index, "args": ", ".join(func_args + build_vararg_list(index))})
			. indent("\t")
		)
...
func build() -> PackedStringArray:
	if fd.name() == "_init":
		return [
			def_constructor.format(
				{"constructor_args": build_constructor_arguments(), "args": ", ".join(func_args)}
			)
		]
