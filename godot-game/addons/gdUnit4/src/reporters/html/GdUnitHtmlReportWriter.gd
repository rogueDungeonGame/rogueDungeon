...
func _write(report_dir: String, report: GdUnitTestSuiteReport) -> String:
	...
	template = GdUnitHtmlPatterns.build(
		template, report, "", GdUnitHtmlPatterns.get_path_as_link(report)
	)

	var report_output_path := GdUnitHtmlPatterns.create_suite_output_path(
		report_dir, report.path(), report.name()
	)
