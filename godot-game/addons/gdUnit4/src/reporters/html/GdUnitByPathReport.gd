...
func write(report_dir: String) -> String:
	...
	var path_report := GdUnitHtmlPatterns.build(
		template, self, output_path, GdUnitHtmlPatterns.get_path_as_link(self)
	)
...
func apply_testsuite_reports(
	...
	for report: GdUnitTestSuiteReport in test_suite_reports:
		var report_link := (
			GdUnitHtmlPatterns
			. create_suite_output_path(report_dir, report.path(), report.name())
			. replace(report_dir, "..")
		)
