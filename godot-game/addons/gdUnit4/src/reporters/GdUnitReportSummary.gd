	...
	for report: GdUnitTestSuiteReport in _reports:
		if report.get_resource_path() == resource_path:
			var test_report := GdUnitTestCaseReport.new(
				resource_path, suite_name, test_name, _text_formatter
			)
...
func add_testsuite_report(p_resource_path: String, p_suite_name: String, p_test_count: int) -> void:
	_reports.append(
		GdUnitTestSuiteReport.new(p_resource_path, p_suite_name, p_test_count, _text_formatter)
	)
...
func update_testsuite_counters(
	...
	for report: GdUnitTestSuiteReport in _reports:
		if report.get_resource_path() == p_resource_path:
			report._update_testsuite_counters(
				p_error_count,
				p_failure_count,
				p_orphan_count,
				p_skipped_count,
