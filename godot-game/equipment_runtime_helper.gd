...
func sanitize_coin_faction_state(state_variant: Variant) -> Dictionary:
	...
	out["auto_destroy_coin_enabled"] = _variant_to_bool(
		state.get("auto_destroy_coin_enabled", false), false
	)
...
func sanitize_inventory_meta_entry(
	item_db: Array, entry_variant: Variant, item_idx: int = -1
) -> Dictionary:
	...
	if entry_variant is Dictionary:
		...
		out["permanent_agility"] = maxi(
			int(entry.get("permanent_agility", out["permanent_agility"])), 0
		)
		out["permanent_intelligence"] = int(
			entry.get("permanent_intelligence", out["permanent_intelligence"])
		)
		...
		out["permanent_damage_tenths"] = int(
			entry.get("permanent_damage_tenths", out["permanent_damage_tenths"])
		)
		out["permanent_attack_speed_percent"] = int(
			entry.get("permanent_attack_speed_percent", out["permanent_attack_speed_percent"])
		)
		out["permanent_spell_damage_percent"] = maxi(
			int(entry.get("permanent_spell_damage_percent", out["permanent_spell_damage_percent"])),
			0
		)
		out["permanent_attack_range"] = maxi(
			int(entry.get("permanent_attack_range", out["permanent_attack_range"])), 0
		)
		out["permanent_physical_crit_multiplier"] = maxi(
			int(
				entry.get(
					"permanent_physical_crit_multiplier", out["permanent_physical_crit_multiplier"]
				)
			),
			0
		)
		out["particle_bonus_per_two_charges"] = maxi(
			int(entry.get("particle_bonus_per_two_charges", out["particle_bonus_per_two_charges"])),
			0
		)
		out["deferred_child_coin_on_destroy"] = maxi(
			int(entry.get("deferred_child_coin_on_destroy", out["deferred_child_coin_on_destroy"])),
			0
		)
	var item_name: String = get_item_name_by_index(item_db, item_idx)
...
func sanitize_inventory_meta_array(
	item_db: Array, meta_variant: Variant, inventory: Array
) -> Array:
	var out: Array = []
	var source: Array = []
...
func get_inventory_meta_entry(
	item_db: Array, meta_array: Array, slot_idx: int, item_idx: int = -1
) -> Dictionary:
...
func set_inventory_meta_entry(
	item_db: Array, meta_array: Array, slot_idx: int, entry: Dictionary, item_idx: int = -1
) -> void:
	if slot_idx < 0:
		return
...
func get_effective_inventory_item_level(
	...
	if (
		...
		and find_item_slots_by_name(item_db, inventory, GOLD_COIN_ITEM_NAME).size() > 0
		and item_name != GOLD_COIN_ITEM_NAME
	):
		return 0
	var base_level: int = _get_item_level(item_db, item_idx)
...
func get_inventory_total_item_level(item_db: Array, inventory: Array, meta_array: Array) -> int:
	...
	for i in range(inventory.size()):
		total += maxi(
			get_effective_inventory_item_level(
				item_db, int(inventory[i]), i, meta_array, inventory
			),
			0
		)
	return total
...
func build_inventory_signature(
	inv: Array,
	owner_key: int,
	runtime_meta: Array,
