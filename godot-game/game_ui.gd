...
var _observing_boss: bool = false
var _observed_enemy_state: Dictionary = {}
...
var _talent_rng := RandomNumberGenerator.new()
...
func _build_current_observe_state() -> Dictionary:
	return {
		"observed_peer_id": _observed_peer_id,
		"observing_boss": _observing_boss,
		"observed_enemy_state": _observed_enemy_state.duplicate(true),
		"self_peer_id": _self_peer_id,
		"observed_remote_hero_state": _observed_remote_hero_state.duplicate(true),
		"observed_remote_equipment_state": _observed_remote_equipment_state.duplicate(true),
	}


func _apply_observe_state_snapshot(state: Dictionary) -> void:
	_observed_peer_id = maxi(int(state.get("observed_peer_id", _observed_peer_id)), 0)
	_observing_boss = _variant_to_bool(state.get("observing_boss", _observing_boss), _observing_boss)
	var enemy_state_variant: Variant = state.get("observed_enemy_state", _observed_enemy_state)
	if enemy_state_variant is Dictionary:
		_observed_enemy_state = (enemy_state_variant as Dictionary).duplicate(true)
	var self_peer_variant: Variant = state.get("self_peer_id", _self_peer_id)
	_self_peer_id = int(self_peer_variant)
	var remote_hero_variant: Variant = state.get("observed_remote_hero_state", _observed_remote_hero_state)
	if remote_hero_variant is Dictionary:
		_observed_remote_hero_state = (remote_hero_variant as Dictionary).duplicate(true)
	var remote_equipment_variant: Variant = state.get("observed_remote_equipment_state", _observed_remote_equipment_state)
	if remote_equipment_variant is Dictionary:
		_observed_remote_equipment_state = (remote_equipment_variant as Dictionary).duplicate(true)
...
func _process(_delta: float) -> void:
	...
	_update_haste_cd()
	_update_e_skill_cd()
	_update_skill_cast_masks()
	_update_r_skill_status()
	_check_shop_click()
	if _shop_visible:
		_update_shop_info()
	_refresh_inventory()
...
func _reset_talent_selection_runtime() -> void:
	...
	_set_talent_popup_visible(false)
	_apply_talent_bonuses_to_hero()
...
func _update_talent_selection_flow() -> void:
	...
	if _is_observing_remote() or _is_observing_boss() or _is_observing_enemy():
		return
	var hero_selected: bool = _variant_to_bool(_hero_ctrl.get("hero_selection_confirmed"), false)
	if not hero_selected:
		if _talent_last_hero_selected:
			_talent_last_hero_selected = false
			_reset_talent_selection_runtime()
		return
	...
	if not _talent_session_initialized:
		...
		_talent_last_seen_hero_level = maxi(_variant_to_int(_hero_ctrl.get("hero_level"), 1), 1)
		_queue_talent_choices(TALENT_CHOICES_PER_LEVEL)
	else:
		var hero_level: int = maxi(_variant_to_int(_hero_ctrl.get("hero_level"), 1), 1)
...
func _set_talent_popup_visible(visible: bool) -> void:
	_get_talent_popup_controller().set_visible(visible)
	_sync_hero_input_lock_by_ui_state()


func _sync_hero_input_lock_by_ui_state() -> void:
	if _hero_ctrl == null or not _hero_ctrl.has_method("set_input_locked_by_ui"):
		return
	var should_lock_input: bool = _shop_visible or _talent_popup_open
	_hero_ctrl.call("set_input_locked_by_ui", should_lock_input)


func _get_current_talent_pool() -> Array:
	if _hero_ctrl == null:
		return []
	var hero_id: int = _variant_to_int(_hero_ctrl.get("hero_id"), 0)
...
func _show_next_talent_popup() -> void:
	var pool: Array = _get_current_talent_pool()
	...
	_set_talent_popup_visible(true)
	_get_talent_popup_controller().show_options(_talent_current_options, _talent_pending_choice_count, _talent_selected_counts, "天赋强化")
...
func _build_talent_bonus_bundle() -> Dictionary:
	if _hero_ctrl == null:
		return {}
	var hero_id: int = _variant_to_int(_hero_ctrl.get("hero_id"), 0)
	if hero_id == HERO_ID_RANGED:
...
func _apply_talent_bonuses_to_hero() -> void:
	if _hero_ctrl == null:
		return
	if _hero_ctrl.has_method("apply_talent_bonuses"):
		_hero_ctrl.call("apply_talent_bonuses", _build_talent_bonus_bundle())
...
func _update_shop_debug_overlay() -> void:
	if _shop_debug_label == null:
		return
	_shop_debug_label.visible = debug_shop_click_logs
	if not debug_shop_click_logs:
		return
	var self_peer_id: int = _self_peer_id
...
func _build_boss_info_section(parent: HBoxContainer) -> void:
	var refs: Dictionary = _get_hud_status_panels_controller().build_boss_section(parent, {
		"boss_panel_bg": Color(0.1, 0.04, 0.04, 1.0),
		"boss_panel_border": Color(0.7, 0.2, 0.15, 1.0),
		"boss_title_color": Color(1.0, 0.4, 0.3, 1.0),
		"boss_portrait_bg": Color(0.08, 0.03, 0.03, 1.0),
		"boss_display_name": BOSS_DISPLAY_NAME,
		"boss_hp_bar_color": Color(0.9, 0.2, 0.15, 1.0),
		"text_color": COLOR_TEXT,
		"text_dim": COLOR_TEXT_DIM,
	})
	_boss_portrait_rect = refs.get("boss_portrait_rect", null) as ColorRect
	_boss_hp_bar = refs.get("boss_hp_bar", null) as ProgressBar
	_boss_hp_label = refs.get("boss_hp_label", null) as Label
...
func _update_skill_cast_masks() -> void:
	_get_skill_status_service().update_skill_cast_masks(
		_hero_ctrl,
		_is_observing_boss(),
		_is_observing_enemy(),
		_is_observing_remote(),
		_skill_mana_masks,
		_skill_cd_masks,
		_skill_cd_mask_materials
	)
...
func set_observed_peer(peer_id: int) -> void:
	...
	if _variant_to_bool(result.get("reset_local_destroy_mode", false), false):
		_destroy_mode = false
...
func _sync_local_equipment_state_from_authority() -> void:
	var authority_state: Dictionary = _get_observe_sync_service().fetch_local_authority_equipment_state(_net_ctrl, _is_observing_remote(), _self_peer_id)
	if authority_state.is_empty():
		return
	if _is_local_equipment_state_synced(authority_state):
		return
...
func _is_local_equipment_state_synced(authority_state: Dictionary) -> bool:
	_ensure_local_equipment_runtime_state()
...
func _is_observing_boss() -> bool:
	return _observing_boss


func _is_observing_enemy() -> bool:
	return not _observed_enemy_state.is_empty()
...
func _update_skill_name_labels() -> void:
	_get_skill_status_service().update_skill_name_labels(
		_hero_ctrl,
		_is_observing_boss(),
		_is_observing_enemy(),
		_is_observing_remote(),
		_skill_button_by_key,
		_q_skill_name_label,
		_w_skill_name_label,
		_e_skill_name_label,
		_r_skill_name_label
...
func _update_hero_info() -> void:
	if _is_observing_boss():
		_update_observed_boss_info()
		return
	if _is_observing_enemy():
		_update_observed_enemy_info()
		return
	...
	if _hero_name_label != null:
		var local_hero_id: int = _variant_to_int(_hero_ctrl.get("hero_id"), 0)
		var local_profile: String = str(_hero_ctrl.get("hero_profile"))
		_hero_name_label.text = "你自己 · %s" % _resolve_hero_display_name(local_hero_id, local_profile)
	...
	if _atk_range_label != null and attack_range != null:
		_atk_range_label.text = "攻距: %d" % roundi(float(attack_range))
	if _cdr_label != null and cooldown_reduction_percent_total != null:
		...
		_set_tooltip_if_changed(_cdr_label, "缩短技能冷却时间（当前作用于 Q/W/R 主动技能）。上限 80%。")
	...
	if spd != null and _spd_label != null:
		_spd_label.text = "移速: %d" % roundi(float(spd))
	if _spd_label != null:
	...
	if str_val != null and _str_label != null:
		_str_label.text = "力量: %d" % _variant_to_int(str_val, 0)
	...
	if agi_val != null and _agi_label != null:
		_agi_label.text = "敏捷: %d" % _variant_to_int(agi_val, 0)
	...
	if int_val != null and _int_label != null:
		_int_label.text = "智力: %d" % _variant_to_int(int_val, 0)
...
func _update_remote_hero_info() -> void:
	...
	if _hero_name_label != null:
		_hero_name_label.text = "玩家P%d · %s" % [_observed_peer_id, _resolve_hero_display_name(remote_hero_id, profile_text)]
...
func _update_observed_boss_info() -> void:
	if _enemy_ai == null or _hero_hp_bar == null:
		return
	if _hero_name_label != null:
		_hero_name_label.text = "BOSS · %s" % BOSS_DISPLAY_NAME

	var current_hp_variant: Variant = _enemy_ai.get("_current_hp")
	var max_hp_variant: Variant = _enemy_ai.get("max_hp")
	...
	_hero_hp_bar.value = clampf(float(boss_current_hp) / float(boss_max_hp) * 100.0, 0.0, 100.0)
	if _hero_hp_label != null:
		_hero_hp_label.text = "%d / %d" % [boss_current_hp, boss_max_hp]
	if _hero_mp_bar != null:
		_hero_mp_bar.value = 0.0
	if _hero_mp_label != null:
		_hero_mp_label.text = "-- / --"

	var boss_damage_variant: Variant = _enemy_ai.get("damage_per_hit")
	if _atk_label != null:
		if boss_damage_variant != null:
			_atk_label.text = "攻击: %d" % int(boss_damage_variant)
		else:
			_atk_label.text = "攻击: -"
		_set_tooltip_if_changed(_atk_label, "")
	if _def_label != null:
		_def_label.text = "护甲: -"
		_set_tooltip_if_changed(_def_label, "")

	var boss_speed_variant: Variant = _enemy_ai.get("move_speed")
	if _spd_label != null:
		if boss_speed_variant != null:
			_spd_label.text = "移速: %d" % int(round(float(boss_speed_variant)))
		else:
			_spd_label.text = "移速: -"
		_set_tooltip_if_changed(_spd_label, "")

	var boss_attack_speed_variant: Variant = _enemy_ai.get("attack_speed")
	var boss_attack_range_variant: Variant = _enemy_ai.get("attack_range")
	if _atk_speed_label != null:
		if boss_attack_speed_variant != null:
			_atk_speed_label.text = "攻速: %.2f" % float(boss_attack_speed_variant)
		else:
			_atk_speed_label.text = "攻速: -"
	if _atk_interval_label != null:
		if boss_attack_speed_variant != null and float(boss_attack_speed_variant) > 0.0001:
			_atk_interval_label.text = "攻间隔: %.2f" % (1.0 / float(boss_attack_speed_variant))
		else:
			_atk_interval_label.text = "攻间隔: -"
	if _atk_range_label != null:
		if boss_attack_range_variant != null:
			_atk_range_label.text = "攻距: %d" % int(round(float(boss_attack_range_variant)))
		else:
			_atk_range_label.text = "攻距: -"
	if _cdr_label != null:
		_cdr_label.text = "冷却减免: -"
		_set_tooltip_if_changed(_cdr_label, "")
	if _phys_crit_rate_label != null:
		_phys_crit_rate_label.text = "物暴率: -"
		_set_tooltip_if_changed(_phys_crit_rate_label, "")
	if _phys_crit_mul_label != null:
		_phys_crit_mul_label.text = "物暴倍: -"
		_set_tooltip_if_changed(_phys_crit_mul_label, "")
	if _spell_crit_rate_label != null:
		_spell_crit_rate_label.text = "法暴率: -"
		_set_tooltip_if_changed(_spell_crit_rate_label, "")
	if _spell_crit_mul_label != null:
		_spell_crit_mul_label.text = "法暴倍: -"
		_set_tooltip_if_changed(_spell_crit_mul_label, "")
	if _hp_regen_label != null:
		_hp_regen_label.text = "回血: -"
	if _mp_regen_label != null:
		_mp_regen_label.text = "回蓝: -"
	if _str_label != null:
		_str_label.text = "力量: -"
		_set_tooltip_if_changed(_str_label, "")
	if _agi_label != null:
		_agi_label.text = "敏捷: -"
		_set_tooltip_if_changed(_agi_label, "")
	if _int_label != null:
		_int_label.text = "智力: -"
		_set_tooltip_if_changed(_int_label, "")


func _update_observed_enemy_info() -> void:
	if _hero_hp_bar == null:
		return
	var enemy_state: Dictionary = _observed_enemy_state
	...
	if _hero_name_label != null:
		...
		_hero_name_label.text = enemy_name

	var enemy_current_hp: int = maxi(int(enemy_state.get("hp", 0)), 0)
	var enemy_max_hp: int = maxi(int(enemy_state.get("max_hp", 1)), 1)
	_hero_hp_bar.value = clampf(float(enemy_current_hp) / float(enemy_max_hp) * 100.0, 0.0, 100.0)
	if _hero_hp_label != null:
		_hero_hp_label.text = "%d / %d" % [enemy_current_hp, enemy_max_hp]
	if _hero_mp_bar != null:
		_hero_mp_bar.value = 0.0
	if _hero_mp_label != null:
		_hero_mp_label.text = "-- / --"
	...
	if _atk_label != null:
		if enemy_state.has("damage"):
			_atk_label.text = "攻击: %d" % int(enemy_state.get("damage", 0))
		else:
			_atk_label.text = "攻击: -"
		_set_tooltip_if_changed(_atk_label, "")
	if _def_label != null:
	...
	if _spd_label != null:
		if enemy_state.has("move_speed"):
			_spd_label.text = "移速: %d" % int(round(float(enemy_state.get("move_speed", 0.0))))
		else:
			_spd_label.text = "移速: -"
		_set_tooltip_if_changed(_spd_label, "")

	if _atk_speed_label != null:
		if enemy_state.has("attack_speed"):
			_atk_speed_label.text = "攻速: %.2f" % float(enemy_state.get("attack_speed", 0.0))
		else:
			_atk_speed_label.text = "攻速: -"
	if _atk_interval_label != null:
		var enemy_attack_speed: float = float(enemy_state.get("attack_speed", 0.0))
		if enemy_attack_speed > 0.0001:
			_atk_interval_label.text = "攻间隔: %.2f" % (1.0 / enemy_attack_speed)
		else:
			_atk_interval_label.text = "攻间隔: -"
	if _atk_range_label != null:
		if enemy_state.has("attack_range"):
			_atk_range_label.text = "攻距: %d" % int(round(float(enemy_state.get("attack_range", 0.0))))
		else:
			_atk_range_label.text = "攻距: -"
	if _cdr_label != null:
		_cdr_label.text = "冷却减免: -"
...
func _update_flash_cd() -> void:
	_get_skill_status_service().update_flash_cd(
		_hero_ctrl,
		_is_observing_boss(),
		_is_observing_enemy(),
		_is_observing_remote(),
...
func _update_haste_cd() -> void:
	_get_skill_status_service().update_haste_cd(
		_hero_ctrl,
		_is_observing_boss(),
		_is_observing_enemy(),
		_is_observing_remote(),
...
func _update_e_skill_cd() -> void:
	_get_skill_status_service().update_e_skill_cd(
		_hero_ctrl,
		_is_observing_boss(),
		_is_observing_enemy(),
		_is_observing_remote(),
...
func _update_r_skill_status() -> void:
	_get_skill_status_service().update_r_skill_status(
		_hero_ctrl,
		_is_observing_boss(),
		_is_observing_enemy(),
		_is_observing_remote(),
...
func _check_shop_click() -> void:
	...
	if clicked == true:
		...
		if debug_shop_click_logs:
			var self_peer_id_debug: int = _self_peer_id
...
func _on_shop_item_mouse_exited(item_panel: Control) -> void:
	if item_panel == null:
		return
	_get_shop_panel_controller().on_item_mouse_exited(item_panel, get_viewport().get_mouse_position())
...
func _get_shop_state_for_owner(owner_peer_id: int) -> Dictionary:
	...
	if _is_observing_remote() and _observed_peer_id == owner_peer_id:
