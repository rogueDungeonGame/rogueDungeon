...
func should_start_with_lobby(
	lobby_enabled: bool, lobby_action: String, _lobby_id_text: String, parsed_lobby_id: int
) -> bool:
...
func handle_lobby_created(
	current_state: Dictionary, connect_status: int, lobby_id: int, is_ok: bool
) -> Dictionary:
	var next_state: Dictionary = _copy_state(current_state)
	if lobby_id <= 0:
		return {
...
func handle_lobby_joined(
	current_state: Dictionary, lobby_id: int, response: int, is_ok: bool
) -> Dictionary:
	var next_state: Dictionary = _copy_state(current_state)
	if lobby_id <= 0:
		return {
	...
	var pending_action: String = (
		str(next_state.get("_steam_lobby_pending_action", "")).strip_edges()
	)
	next_state["_steam_lobby_id"] = lobby_id
	next_state["steam_lobby_id_text"] = str(lobby_id)
	next_state["_steam_lobby_pending_action"] = ""
...
func apply_backend_snapshot(
	current_state: Dictionary, next_owner: int, next_members: Array
) -> Dictionary:
	var next_state: Dictionary = _copy_state(current_state)
	var prev_owner: int = int(next_state.get("_steam_lobby_owner_steam_id", 0))
	var current_members: Array = _int_array_copy(
		next_state.get("_steam_lobby_member_steam_ids", [])
	)
...
func pick_successor_owner(
	lobby_id: int, local_steam_id: int, current_owner_steam_id: int, member_steam_ids: Array
) -> int:
	if lobby_id <= 0:
		return 0
...
func advance_lobby_poll(
	lobby_id: int,
	poll_elapsed_sec: float,
	delta: float,
...
func build_member_data_payload(
	local_steam_id: int, hero_selected: bool, hero_profile: String
) -> Dictionary:
	return {
...
func _copy_state(state: Dictionary) -> Dictionary:
	return {
		...
		_int_array_copy(state.get("_steam_lobby_member_steam_ids", [])),
		...
		"_migration_pause_world_authority":
		bool(state.get("_migration_pause_world_authority", false)),
