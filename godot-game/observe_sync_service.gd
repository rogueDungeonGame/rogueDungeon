...
func apply_observed_peer(current_state: Dictionary, peer_id: int) -> Dictionary:
	...
	if (
		int(next_state.get("observed_peer_id", 0)) == safe_peer_id
		and not should_clear_boss
		and observed_enemy_state.is_empty()
	):
		return {
			"changed": false,
			"state": next_state,
...
func apply_observed_boss(current_state: Dictionary, enabled: bool) -> Dictionary:
	...
	var current_observing_boss: bool = _bool_from_variant(
		next_state.get("observing_boss", false), false
	)
	...
	if (
		current_observing_boss == should_observe_boss
		and (
			not should_observe_boss
			or (current_observed_peer_id == 0 and current_enemy_state.is_empty())
		)
	):
		return {
			"changed": false,
...
func update_network_view_state(net_ctrl: Node, current_state: Dictionary) -> Dictionary:
	...
	if (
		...
		or not _dict_copy(next_state.get("observed_enemy_state", {})).is_empty()
	):
		next_state["observed_remote_hero_state"] = {}
		next_state["observed_remote_equipment_state"] = {}
		return {
...
func is_local_equipment_state_synced(authority_state: Dictionary, local_state: Dictionary) -> bool:
	...
	var authority_inventory: Array = _int_array_copy(
		local_state.get("authority_inventory", authority_state.get("inventory", []))
	)
	if not _int_array_equals(local_inventory, authority_inventory):
		return false
	if (
		int(authority_state.get("gold", local_state.get("gold", 0)))
		!= int(local_state.get("gold", 0))
	):
		return false
	var authority_shop_level: int = clampi(
		int(authority_state.get("shop_level", local_state.get("shop_level", 1))), 1, 7
	)
	...
	if (
		_bool_from_variant(
			authority_state.get("destroy_mode", local_state.get("destroy_mode", false)),
			_bool_from_variant(local_state.get("destroy_mode", false), false)
		)
		!= _bool_from_variant(local_state.get("destroy_mode", false), false)
	):
		return false
	if (
		JSON.stringify(
			_copy_array(
				local_state.get(
					"authority_inventory_meta", authority_state.get("inventory_meta", [])
				)
			)
		)
		!= JSON.stringify(_copy_array(local_state.get("inventory_meta", [])))
	):
		return false
	if (
		JSON.stringify(
			_dict_copy(
				local_state.get(
					"authority_destroy_faction_state",
					authority_state.get("destroy_faction_state", {})
				)
			)
		)
		!= JSON.stringify(_dict_copy(local_state.get("destroy_faction_state", {})))
	):
		return false
	if (
		JSON.stringify(
			_dict_copy(
				local_state.get(
					"authority_coin_faction_state", authority_state.get("coin_faction_state", {})
				)
			)
		)
		!= JSON.stringify(_dict_copy(local_state.get("coin_faction_state", {})))
	):
		return false
	return true
...
func _copy_state(state: Dictionary) -> Dictionary:
	return {
		...
		"observed_remote_equipment_state":
		_dict_copy(state.get("observed_remote_equipment_state", {})),
	}
