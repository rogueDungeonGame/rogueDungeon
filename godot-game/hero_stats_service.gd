...
static func get_primary_attr_value(
	primary_attribute: String, strength: int, agility: int, intelligence: int
) -> int:
...
static func build_recalculated_stats(input: Dictionary) -> Dictionary:
	...
	var strength: int = (
		int(
			round(
				(
					...
					+ (
						(
							float(input.get("strength_growth", 0.0))
							+ float(input.get("strength_growth_bonus", 0.0))
						)
						* float(lv - 1)
					)
				)
		...
		+ int(input.get("equip_strength_bonus", 0))
		+ int(input.get("battle_banner_applied_strength_bonus", 0))
	)
	var agility: int = (
		int(
			round(
				(
					...
					+ (
						(
							float(input.get("agility_growth", 0.0))
							+ float(input.get("agility_growth_bonus", 0.0))
						)
						* float(lv - 1)
					)
				)
		...
		+ int(input.get("equip_agility_bonus", 0))
		+ int(input.get("settlement_permanent_agility_bonus", 0))
		+ int(input.get("battle_banner_applied_agility_bonus", 0))
	)
	var intelligence: int = (
		...
		+ int(input.get("equip_intelligence_bonus", 0))
		+ int(input.get("necro_bonus_int_from_flute", 0))
		+ int(input.get("settlement_permanent_intelligence_bonus", 0))
		+ int(input.get("battle_banner_applied_intelligence_bonus", 0))
	)

	var max_hp: int = (
		int(input.get("base_hp_flat", 0))
	...
	var scaled_base_mana: int = roundi(
		(
			float(input.get("base_mana_flat", 0))
			* maxf(float(input.get("initial_mana_multiplier", 0.0)), 0.0)
		)
	)
	var max_mana: int = (
		scaled_base_mana
		+ int(input.get("equip_mana_bonus", 0))
		+ intelligence * int(input.get("int_mana_per_point", 15))
	)

	var damage_per_hit: int = (
		int(input.get("base_damage_flat", 0))
		+ int(input.get("equip_damage_bonus", 0))
		+ int(input.get("battle_banner_applied_damage_bonus", 0))
	)
	damage_per_hit += get_primary_attr_value(
		str(input.get("primary_attribute", "敏捷")), strength, agility, intelligence
	)
	...
	var armor: float = (
		float(input.get("base_armor_flat", 0.0))
		+ float(input.get("equip_armor_bonus", 0.0))
		+ float(agility) * float(input.get("agi_armor_per_point", 0.0))
	)
	var raw_physical_crit_chance: float = float(
		input.get("settlement_raw_physical_crit_chance", 0.0)
	)
	var dynamic_physical_crit_multiplier_bonus: float = float(
		input.get("settlement_dynamic_physical_crit_multiplier_bonus", 0.0)
	)
	...
	var physical_crit_multiplier: float = maxf(
		(
			...
			+ (
				(
					float(input.get("equip_physical_crit_multiplier_bonus", 0.0))
					+ dynamic_physical_crit_multiplier_bonus
				)
				* 0.01
			)
		),
	...
	var spell_crit_chance: float = clampf(
		(
			float(input.get("base_spell_crit_chance", 0.0))
			+ float(input.get("equip_spell_crit_chance_bonus", 0.0))
		),
		0.0,
		100.0
	)
	var spell_crit_multiplier: float = maxf(
		(
			float(input.get("base_spell_crit_multiplier", 2.0))
			+ float(input.get("equip_spell_crit_multiplier_bonus", 0.0)) * 0.01
		),
		1.0
	)
	if bool(input.get("is_warden", false)):
	...
	var hp_regen_per_second: float = (
		float(input.get("base_hp_regen_flat", 0.0))
		+ float(input.get("equip_hp_regen_bonus", 0.0))
		+ float(strength) * float(input.get("str_hp_regen_per_point", 0.05))
	)
	var mana_regen_per_second: float = (
		float(input.get("base_mana_regen_flat", 0.0))
		+ float(intelligence) * float(input.get("int_mana_regen_per_point", 0.05))
	)
	var move_speed: float = clampf(
		(
			float(input.get("base_move_speed", 0.0))
			+ float(input.get("equip_move_speed_bonus", 0.0))
			+ float(input.get("talent_move_speed_bonus_flat", 0.0))
		),
		float(input.get("wc3_min_move_speed", 100.0)),
		float(input.get("wc3_max_move_speed", 522.0))
	)
	var attack_range: float = maxf(
		(
			float(input.get("base_attack_range", 0.0))
			+ float(input.get("equip_attack_range_bonus", 0.0))
			+ float(input.get("talent_attack_range_bonus_flat", 0.0))
		),
		60.0
	)
...
static func get_attack_interval(
	attack_speed_scale: float, spark_attack_interval_reduction_sec: float
) -> float:
...
static func compute_cooldown_reduction_percent(
	base_cooldown_reduction_percent: float,
	equip_cooldown_reduction_percent_bonus: float,
	max_cooldown_reduction_percent: float
) -> float:
...
static func compute_skill_cooldown(
	base_cooldown: float,
	...
	if safe_base <= 0.0:
		return 0.0
	var cdr_ratio: float = (
		clampf(cooldown_reduction_percent_total, 0.0, max_cooldown_reduction_percent) * 0.01
	)
	return safe_base * maxf(1.0 - cdr_ratio, 0.0)


static func get_armor_damage_multiplier(
	armor: float, armor_k_melee_default: float, negative_armor_base_melee_default: float
) -> float:
	if armor >= 0.0:
		var reduction: float = (
			(armor_k_melee_default * armor) / (1.0 + armor_k_melee_default * armor)
		)
...
static func compute_physical_damage(
	base_damage_amount: int, physical_crit_chance: float, physical_crit_multiplier: float
) -> int:
	return int(
		(
			compute_physical_damage_result(
...
static func compute_spell_damage(
	base_damage_amount: int,
	spell_bonus_percent: float,
	spell_crit_chance: float,
	spell_crit_multiplier: float
) -> int:
	return int(
		(
			compute_spell_damage_result(
				base_damage_amount, spell_bonus_percent, spell_crit_chance, spell_crit_multiplier
			)
			. get("damage", 0)
		)
	)
...
static func compute_physical_damage_result(
	base_damage_amount: int, physical_crit_chance: float, physical_crit_multiplier: float
) -> Dictionary:
...
static func compute_spell_damage_result(
	base_damage_amount: int,
	spell_bonus_percent: float,
	spell_crit_chance: float,
	spell_crit_multiplier: float
